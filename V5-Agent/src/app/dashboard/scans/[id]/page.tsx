      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="relative flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="space-y-4">
              ...
              <h1 className="text-4xl font-bold tracking-tight text-white flex items-center gap-4">
                ...
                {scan.url ? (new URL(scan.url).hostname) : "Scanning Target"}
              </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-black border border-white/[0.06] rounded-2xl p-6 font-mono text-xs space-y-2 h-[500px] overflow-y-auto scrollbar-hide bg-[#050505]">
              {scan.logs?.map((log: string, i: number) => (
                <div key={i} className="flex gap-4 group">
                  <span className="text-slate-700 min-w-[30px] select-none text-right">{(i + 1).toString().padStart(2, '0')}</span>
                  <span className={`flex-1 ${log.includes('[Finding]') ? 'text-blue-400 font-bold' : log.includes('COMPLETE') ? 'text-green-400' : 'text-slate-400'}`}>
                    {log}
                  </span>
                </div>
              ))}
              {scan.status === "RUNNING" && (
                <div className="flex gap-4 animate-pulse">
                  <span className="text-slate-700 min-w-[30px] text-right">{(scan.logs?.length || 0) + 1}</span>
              ...
              {scan.findings?.length === 0 ? (
                ...
                scan.findings.sort((a: any, b: any) => {
                  ...
                  return (
                    ...
                    >
                      {/* Severity Accent */}
                      <div className={`h-[2px] bg-${sevColor}-500`} />
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="bg-white/[0.02] rounded-xl p-3 space-y-2">
                            ...
                            <div className="h-1 bg-white/[0.04] rounded-full overflow-hidden">
                              <div className="h-full bg-slate-500 rounded-full" style={{ width: `${exploitScore}%` }} />
                            </div>
                          </div>
                          <div className="bg-white/[0.02] rounded-xl p-3 space-y-2">
                            <div className="flex justify-between text-[8px] font-black uppercase tracking-[0.2em]">
                            ...
                            <div className="h-1 bg-white/[0.04] rounded-full overflow-hidden">
                              <div className="h-full bg-slate-500 rounded-full" style={{ width: `${impactScore}%` }} />
                            </div>
                          </div>
                        </div>
